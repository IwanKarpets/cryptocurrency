...
export const addToPortfolio = (items, currency) => (dispatch) => {
    ...
    items.forEach((cp) => {
      if (cp.id === currency.id) {
        ...
        dispatch({ type:SET_IS_ADDED, payload: true})
        setTimeout(()=>{
    ...
    dispatch({ type: ADD_TO_PORTFOLIO, payload: {items: items } });
  };
  ...
  export const removeFromPortfolio = (items, id) => (dispatch) => {
    ...
    dispatch({ type: REMOVE_FROM_PORTFOLIO, payload: {items: filterItems } });
  };
